...
위 두 가지를 결합하면 더 일반적인 벨만 방정식을 얻을 수 있습니다.
...
상태 s1에서 시작하여 상태 s2로 갈 수 있고 이 때 보상은 1 입니다. s2에서는 s1로 갈 수 있고 보상은 2 입니다. 이 에이전트의 상태 시퀀스는 s1, s2의 반복일 것입니다.
...
실용적인 부분에서 이 방법은 몇 가지 명백한 제한점을 지닙니다.

1. 상태 공간은 이산적(discrete)이어야 하고, 모든 상태에 대해 여러번 반복을 돌일 정도로 충분히 작아야 합니다. 이번 예시에서 사용할 FrozenLake와 같은 환경에서는 문제가 되지 않습니다.

2. 실제에선 행동과 보상 행렬을 위한 전이 확률을 알 경우가 거의 없다는 것입니다. 
